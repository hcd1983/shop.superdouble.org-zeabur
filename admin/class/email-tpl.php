...
class AllroverServiceMail{
	...
	function buyMail(){
		...
		if(!$useLaravel && isset($wordpress_setting["active_mail"]) && $wordpress_setting["active_mail"] == 1 &&  isset($wordpress_setting["mail_url"]) && $wordpress_setting["mail_url"] != ""){
			$WP_OrderInfo=array(
				"OrderNo"=>$this-> OrderNo,
		...
		if(strpos($orderinfo["SendStatus"], 'S1') === false || isset($buy) && $buy -> force == true){
			if ($useLaravel) {
                $email = $sentto;
                $resault = laravelCustomerMail ($OrderNo, $email);
            }else{
