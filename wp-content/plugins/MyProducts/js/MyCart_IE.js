	...
	function AjaxShippingFeeAndDiscount(items){
		...
		jQuery.ajax({
	        ...
	        type: 'POST'
			  ...
			  var Myshippingfee=numberWithCommas(data.shippingfee);
			  var Mydiscount=numberWithCommas(data.discount);	
...
*/
	function GetProductsList(callback){
		...
		jQuery.ajax({
	        url: MyCart.setting.json_url+"?picsize=thumbnail",
	        dataType: 'json',
	        data: {
	           picsize:"thumbnail"
	        },
...
*/
	function AjaxGetProductsList(args,callback){

		jQuery.ajax({
	        url: MyCart.setting.json_url,
	        dataType: 'json',
	        data: {
	           picsize:"thumbnail",
		       args:args
...
*/
	function AjaxaddToCartById(id,amount){
		...
		jQuery.ajax({
	    	...
	    	addItemToCart(data[0],amount); 	  
		});
	}



	var callbackFn=function(callback,b){
		if(typeof(callback) != "function"){
			return;
		}
	...
	var FBpixel = function(action,items){
		...
		switch (action) {
			...
			case 'Purchase':
				...
				if(MyCart.usecate==false){
					fbq('track', 'Purchase',{value: totalBill});
					break;
				}
			...
			case 'InitiateCheckout':
				...
				if(MyCart.usecate==false){
					fbq('track', 'InitiateCheckout',{value: totalBill});
					break;
				}
	...
	var renderCart =function() {
		...
		for(var i=0;i<items.length;i++){
			...
			output += '<div class="top-cart-item clearfix">\
										...
										<div class="top-cart-item-desc">\
											...
											<span class="top-cart-item-price">$'+numberWithCommas(item.price)+'</span>\
		...
		$('#top-cart .total-bill').text(numberWithCommas(totalBill));
	...
	var removeItem= function(remove_int,render){
	...
	var addToCartById = function(id){
		...
		var target = productsList.filter(function(item){ return item.id == id })[0];
	  	...
	  	var item = {
  			...
  			price: target.price,
  			title: target.title,
  			imageUrl: target.all_image_size.thumbnail.url,
  		}
...
*/		
  ...
  var GetProductDataById = function(id,_target){
  ...
  var GetProductImageSizesById = function(id,size){
  ...
  var addItemToCart = function (newItem,amount){
	...
	function CartMoneyFormat(x,custom){
		...
		return moneylogo+numberWithCommas(x);
	...
	function SetCartPage(is_mobile,CheckPage){
		var html;
		...
		html='<div id="cart-list" class="cart item-list">'+
				...
				'<div id="cart-count">'+
					...
					'<div class="total-bill">商品金額: <span class="amount_product"></span></div>'+
			...
			html='<div id="cart-list" class="cart item-list">'+
					...
					'<div id="cart-count">'+
						...
						'<div class="the_shipping_selector">'+MyCart.ShippingSelector+'</div>'+
						'<div class="total-bill">商品金額: <span class="amount_product"></span></div>'+
						'<div class="total-bill total-bill-shipping">運費: <span class="amount_shipping-bill"></span></div>'+
						'<div class="total-bill total-bill-discount">折扣: <span class="amount_discount-bill"></span></div>'+
						'<a id="check-btn" class="button" href="'+CheckPage+'" >'+
						'前往結帳'+
						'</a>'+	
	...
	function SetCheckPage(is_mobile){
		var html;
		...
		html='<div id="check-list" class="cart item-list">'+
				...
				'</div>'+
				'<div id="check-count">'+
					'<div class="total-bill"><div class="label">商品總價:</div> <span class="amount_product"></span></div>'+
					'<div class="total-bill total-bill-shipping"><div class="label">運費:</div> <span class="amount_shipping"></span></div>'+
					'<div class="total-bill total-bill-discount"><div class="label">折扣:</div> <span class="amount_discount"></span></div>'+
					'<div class="total-bill total-price"><div class="label">總金額:</div> <span class="amount_total"></span></div>'+
	...
	function RenderCartList(items){
		...
		if(items.length==0){
			$('.cart-list-container').html("<h2>無資料</h3>");
			return "無資料";
		}
    	...
    	//$('table.billing #shipping_fee').text(numberWithCommas(shippingBill));
    	//$('table.billing #amount_total').text(numberWithCommas(plusShipping));
    ...
    function RenderCheckList(items,shippingfee,discount){
